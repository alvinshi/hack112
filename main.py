...
def musicPlayer():
    while True:
        play_wav("The_Swamp_Whistler.wav")
# player 1
import math
import string
import copy
...
class Grid(object):
    ...
    def __init__(self, w, h):
        ...
        self.brickcolors = [None, "gold", "dark orange", "red", "maroon"]
    ...
    def collide(self, weapon):
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                if (self.brick_collide(weapon.x, weapon.y, 50, 50, r, c)):
    ...
    def draw(self, canvas, data):
        if (self.brick_falling):
            for i in range(len(self.falling_bricks)):
                ...
                color = self.brickcolors[self.falling_bricks[i]]
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                x1, y1 = self.brick_pos(r, c)
                color = self.brickcolors[self.bricks[r][c]]
...
def initGame(data):
    ...
    #data.player1Img = PhotoImage(file="player1.gif")
    #data.player2Img = PhotoImage(file="player2.gif")
    data.photo["gameBackground"] = PhotoImage(file="gameBackground.gif")
    data.rota=0
...
#####################################
def timerFiredVideo(data):
    data.videoTimer -= 10
    if data.videoTimer <= 0:
        data.mode = 'welcome'
        data.timerDelay = 1000
...
def timerFired(data):
    if data.mode == 'game':
        ...
        timerFiredVideo(data)
    elif data.mode == 'loading':
        timerFiredLoading(data)

#####################################
def redrawVideo(canvas, data):
    if data.video:
        for i in range(100):
            data.photo[str(i)] = PhotoImage(file = "Image/Image.0101.gif")
        data.video = False
    canvas.create_image(0,0,anchor = NW, image = data.photo[str(data.videoTimer)])
...
def redrawGame(canvas, data):
    canvas.create_image(0,0,anchor = NW, image=data.photo["gameBackground"])
    data.player2.draw(canvas, data)
    if data.player1!=None:
        data.player1.drawWeapon(canvas, data)
    text = "Player 1" if data.currentPlayer == "player1" else "Player 2"
    canvas.create_text(data.width//2, 55, text = text, font = "Impact 25", fill = "Black")
    canvas.create_text(data.width//2, 53, text = text, font = "Impact 25", fill = "White")
 
def redrawEnd(canvas,data):
    canvas.create_text(data.width//2,data.height//2,text=data.prePlayer+' win', font='impact 40')
def redrawAll(canvas, data):
    if data.mode == 'video':
        redrawVideo(canvas, data)
    if data.mode == 'welcome':
        redrawWelcome(canvas, data)
    elif data.mode == 'loading':
        redrawLoading(canvas, data)
    elif data.mode == 'game':
        data.prePlayer=data.currentPlayer
